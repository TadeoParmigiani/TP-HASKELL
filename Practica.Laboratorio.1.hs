...
donde 0 <= a, b, c, d <= 'n'
-}
cuadrupla n = [(a,b,c,d) | a <- [0..n],
                           b <- [0..n],
                           c <- [0..n], 
                           d <- [0..n], a^2 + b^2 == c^2 + d^2 ]
{-
...
codes [] = 0
codes (x:xs) = (mod x y) : resto mod xs

e) 'restos', que calcula la lista de los restos de la
división de los elementos de una lista de números dada por otro
número dado
 resto [] y = 0
 restp (x:xs) y = (mod x y) : resto mod xs

f) 'cuadrados', que dada una lista de números, devuelva la
lista de sus cuadrados

cuadrados [] = []
cuadrados (x:xs) = [(x*x) : cuadrado xs]
